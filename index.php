...
<div class="wrapper">
      ...
      <div class="collapse navbar-collapse" id="navbarNavDropdown">
        <ul class="navbar-nav me-auto">
          ...
          <li class="nav-item dropdown">
            <a class="nav-link dropdown-toggle" href="#" role="button" data-bs-toggle="dropdown">메뉴1</a>
  ...
  <div class="container mt-2">
    ...
    <?php else: ?>
      <form class="d-flex justify-content-end" method="post" action="index.php?cmd=login">
        <input class="form-control form-control-sm me-2" type="text" name="name" placeholder="ID" required>
  ...
  <main class="container mt-4">
    ...
    if ($cmd === 'login' && $_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        // 임시 로그인 로직 (실제 환경에서는 DB 연동 및 비밀번호 해시 필수)
        if ($id === 'admin' && $pw === '1234') {
        ...
        session_unset();
        session_destroy();
        echo "<div class='alert alert-info'>로그아웃 되었습니다.</div>";
    } elseif ($cmd) {
        $file = $cmd . '.php';
        if (file_exists($file)) {
            include($file);
